...
class N002_Insert_OriginatorIdValidationScenario
    extends BaseSpec
    with GuiceOneServerPerSuite
    with TestDataNotification {

  Feature(
    "UCL_TC_N002 : Insert Request_MDTP returns 403 to DWP on request header - 'gov-uk-originator-id' validation failure"
  ) {
