...
class N006HipInternalServerErrorScenario extends BaseSpec with GuiceOneServerPerSuite with TestDataNotification {
  ...
  Feature(
    "UCL_TC_N006 : API successfully process a valid UCL Notification received from DWP but returns 500 when unexpected internal error occur in HIP"
  ) {

    val cases: Seq[(String, JsValue)] = Seq(
      (
        "Error : Insert request returns 500 to DWP when internal error occur 500 in HIP",
        insertNotificationPayload(nino = ninoWithPrefix("XY500"))
      ),
      (
        "Error : Terminate request handles internal server error from HIP",
