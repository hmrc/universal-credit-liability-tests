...
class N004TerminateSchemaValidationScenario extends BaseSpec with GuiceOneServerPerSuite with TestDataNotification {
  ...
  Feature(
    "UCL_TC_N004 : Terminate_MDTP returns 400 with error response body to DWP on request schema validation failure"
  ) {

    val cases: Seq[(String, Seq[(String, String)], JsValue, ResponseErrorMessage)] = Seq(
      (
    ...
    cases.foreach { case (scenarioName, headers, payload, errorResponseMessage) =>
      Scenario(scenarioName) {
        ...
        Given("MDTP receives a request with invalid request body from DWP")
