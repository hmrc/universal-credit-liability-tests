...
class InsertUnprocessableEntity extends BaseSpec with GuiceOneServerPerSuite with TestDataHip {
  ...
  Feature("422 Insert Unprocessable Entity scenarios") {
    ...
    val cases: Seq[(String, String, String, String)] = Seq(
      (
    ...
    cases.foreach { case (scenarioName, nino, expCode, expReason) =>
      Scenario(s"$scenarioName: Should return 422 with code $expCode") {
        Given("the Universal Credit API is up and running")
        When("a request is sent")

        val response = apiService.postHipUcLiability(validHeaders, nino = nino, requestBody = insertHipPayload())
