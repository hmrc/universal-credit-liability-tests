...
class InsertHip_Unprocessable_Entity extends BaseSpec with GuiceOneServerPerSuite with TestDataHip {
  ...
  Feature("422 Insert Unprocessable Entity scenarios") {
    ...
    cases.foreach { case (scenarioName, headers, payload, expCode, expReason) =>
      Scenario(s"$scenarioName: Should return 422 with code $expCode") {
        ...
        val response = apiService.postHipUcLiability(headers, randomNino, requestBody = payload)
