...
class N002TerminateOriginatorIdValidationScenario
    extends BaseSpec
    with GuiceOneServerPerSuite
    with TestDataNotification {

  Feature(
    "UCL_TC_N002 : Terminate_HIP fails to process the request from API when GovUkOriginatorId validation fails and returns 403 to API and API cascades the response to DWP"
  ) {
