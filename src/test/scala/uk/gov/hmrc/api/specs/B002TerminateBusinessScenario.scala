...
class B002TerminateBusinessScenario extends BaseSpec with GuiceOneServerPerSuite with TestDataNotification {
  ...
  Feature("UCL_TC_B002 : Terminate handle and cascade 422 and business error code from HIP to DWP") {
    ...
    val cases: Seq[(String, NinoPrefix, BusinessErrorCode, BusinessErrorMessage)] = Seq(
      ...
      (
        "Error : API cascade business error 64996 from HIP to DWP",
        "EK310",
        "64996",
      ...
      (
        "Error : API cascade business error 55039 from HIP to DWP",
        "GP050",
        "55039",
